...
func TestBiInt(t *testing.T) {
	testCases := []struct {
		num, den, res float64
	}{
		{-12.0e0, 1.0e0, 0.17660819031554631869e-1},
	...
	for _, tc := range testCases {
		ζ := BiInt(tc.num / tc.den)
		if soclose(ζ, tc.res, 3e-14) == false {
			t.Fatalf("BiInt(%v): expected %v, got %v", tc.num/tc.den, tc.res, ζ)
		}
